...
docker run -d --rm --name "sandbox_piggy_$name" -p "127.0.0.1:$port:3000" piggy
`
...
docker kill "sandbox_piggy_$name"
`
...
DOCKERS_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep "sandbox_piggy_") || true
...
type config struct {
	Listen string `default:"0.0.0.0:9090" split_words:"true"`
