...
docker run -d -e FLAG -e TIMEOUT --rm --name "sandbox_csp_$name" -p "127.0.0.1:$port:80" another-another-csp
`
...
DOCKERS_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep "sandbox_csp_") || true
