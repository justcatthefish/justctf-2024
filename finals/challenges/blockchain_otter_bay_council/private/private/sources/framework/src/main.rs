...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let modules = [ "otter_bay_council", "shadow_coin" ];
    ...
    let chall_addr = match sui_ctf_framework::publish_compiled_module(
        ...
        mncp_modules,
        chall_dependencies,
        Some(String::from("grand_clawmaster")),
    ).await {
        Some(addr) => addr,
        None => {
    ...
    */
    ...
    let deny_cap = SuiValue::Object(FakeID::Enumerated(1, 4), None);
    ...
    let player_addr = SuiValue::MoveValue(MoveValue::Address(
        adapter.compiled_state().resolve_named_address("slick_paw"))
    );
    args_reg.push(admin_cap.clone());       // _: OtterBayCouncilCap
    args_reg.push(player_addr.clone());     // receiver: address
    args_reg.push(treasury_cap.clone());    // treasury_cap: &mut TreasuryCap<SHADOW_COIN>
    args_reg.push(deny_cap.clone());        // deny_cap: &mut DenyCapV2<SHADOW_COIN>
    ...
    let ret_val = match sui_ctf_framework::call_function(
        ...
        chall_addr,
        "otter_bay_council",
        "issue_forbidden_funds",
        args_reg,
        type_args,
        Some("grand_clawmaster".to_string()),
    ).await {
        Ok(output) => output,
        Err(e) => {
            let _ = adapter.cleanup_resources().await;
            println!("[SERVER] error: {e}");
            return Err("error during call to otter_bay_council::issue_forbidden_funds".into())
        }
    };
    ...
    // check solution
    let otter_bay_council_vault = SuiValue::Object(FakeID::Enumerated(1, 1), None);
    let args_check: Vec<SuiValue> = vec![ otter_bay_council_vault ];
