...
docker run -d --rm --name "sandbox_myczek_$name" -p "127.0.0.1:$port:3000" myczek
`
...
docker kill "sandbox_myczek_$name"
`
...
DOCKERS_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep "sandbox_myczek_") || true
