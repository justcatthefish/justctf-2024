...
const FormGet2 = `
You already have a sandbox at <a href="%s">%s</a>.<br>
...
<br>
Here is your instance: <a href="%s">%s</a><br>
...
func SandboxHandler(d *db) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "GET" {
			...
			if userData.Token == "" {
				...
				fmt.Fprintf(w, FormGet2, 
					fmt.Sprintf("http://%s:%s", hostURL.Hostname(), userData.TargetURL.Port()), 
					fmt.Sprintf("http://%s:%s", hostURL.Hostname(), userData.TargetURL.Port()), 
		...
		fmt.Fprintf(w, FormCreated,
			fmt.Sprintf("http://%s:%d", hostURL.Hostname(), randomPort),
			fmt.Sprintf("http://%s:%d", hostURL.Hostname(), randomPort),
