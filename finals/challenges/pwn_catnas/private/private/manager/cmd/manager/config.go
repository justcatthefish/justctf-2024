...
docker run -d --rm --name "sandbox_catnas_$name" -p "0.0.0.0:$port:8000" catnas
`
...
docker kill "sandbox_catnas_$name"
`
...
DOCKERS_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep "sandbox_catnas_") || true
