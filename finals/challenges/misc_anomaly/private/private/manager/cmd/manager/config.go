...
docker run -d --rm --name "sandbox_anomaly_$name" -p "0.0.0.0:$port:2222" anomaly
`
...
docker kill "sandbox_anomaly_$name"
`
...
DOCKERS_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep "sandbox_anomaly_") || true
